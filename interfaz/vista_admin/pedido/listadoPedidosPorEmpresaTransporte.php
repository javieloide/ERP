...
<body>
<h2>Listado pedidos por empresa que transporta</h2>
<!--Sería conveniente utilizar AJAX en este script para recargar-->
<form name="formularioFiltro" method="POST" action="../vista_admin/IndexAdmin.php?principal=pedido/listadoPedidosPorEmpresaTransporte.php">
	
<div class="form-group"> 
                    <h3>codigo empresa:</h3>   
                    <select name="empresaTransporte" class="custom-select">
						<?php
							...
							$todosCodigosEmpresaTransporte = $tPedido->getCodigosEmpresaTransporte();
							
                            foreach ($todosCodigosEmpresaTransporte as $idEmpresaTransporte) {
                                	echo '<option value="'.$idEmpresaTransporte.'">'.$idEmpresaTransporte.'</option>';
                            }
                                echo '<option id="seleccionado" value="'.$idEmpresaTransporte.'" selected>...</option>';
                        ?>
					</select>			
</div>
...
if(isset($_POST['buscar'])){
	...
	$tablaPedidos = $tp->getPedidosPorEmpresaTransporte($_POST['empresaTransporte']);
