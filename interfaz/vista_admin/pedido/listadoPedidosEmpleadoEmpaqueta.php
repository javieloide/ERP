...
<body>
<h2>Listado pedidos por Empleado que empaqueta</h2>
<!--Sería conveniente utilizar AJAX en este script para recargar-->
<form name="formularioFiltro" method="POST" action="../vista_admin/IndexAdmin.php?principal=pedido/listadoPedidosEmpleadoEmpaqueta.php">
	<div class="form-group"> 
                    <h3>nif del empleado:</h3>   
                    <select name="empleado" class="custom-select">
						<?php
							...
							$todosEmpleados = $tEmpleado->getEmpleadosTodos();
                            foreach ($todosEmpleados as $f) {
								$idEmpleado=$f->getIdEmpleado();
								$nif=$f->getNif();
                                echo '<option value="'.$idEmpleado.'">'.$nif.'</option>';

                            }
                                echo '<option id="seleccionado" value="'.$idEmpleado.'" selected>...</option>';
                        ?>
                    </select>
    </div> 
	<div class="form-row">
...
if(isset($_POST['buscar'])){
	...
	$tablaPedidos = $tp->getPedidosPorEmpleadoEmpaqueta($_POST['empleado']);
